...
const BookDetailsPage = () => {
  ...
  return (
    <div className="min-h-screen py-8 px-4 sm:px-6 lg:px-8 bg-[#FDF7F7]"> {/* Coquette theme background */}
      <div className="max-w-6xl mx-auto">
     

        {/* Book Info Card - Always visible */}
        <Card className="mb-6 overflow-hidden shadow-xl bg-white/90"> {/* Semi-transparent white */}
          <div className="p-4 flex justify-between items-center cursor-pointer"
            ...
            /> 
              <h2 className="text-xl font-semibold text-[#9D6B6B]">{bookDetails.title}</h2> {/* Darker Coquette shade */}
            </div>
            {isCollapsed ? 
              ...
              <div className="md:w-3/4 md:pl-8">
                ...
                <div className="flex flex-wrap gap-2 mb-6">
                  {bookDetails.genre.map((tag) => (
                    <Badge 
                      key={tag} 
                      variant="secondary" 
                      className="text-sm bg-[#FFE9E9] text-[#9D6B6B]" // Coquette theme colors
                    >
                      {tag}
                    </Badge>
                  ))}
                ...
                <div className="grid grid-cols-2 md:grid-cols-3 gap-4">
                  <div>
                    ...
                    <p className="font-semibold capitalize text-[#9D6B6B]">{bookDetails.status}</p>
                  </div>
                  <div>
                    <p className="text-sm text-[#D4A5A5]">Release Date</p>
                    <p className="font-semibold text-[#9D6B6B]">{bookDetails.releaseDate}</p>
                  </div>
                  <div>
                    <p className="text-sm text-[#D4A5A5]">Pages</p>
                    <p className="font-semibold text-[#9D6B6B]">{bookDetails.pages}</p>
                  </div>
                </div>
              </div>
            </div>
